...
import os
import time
from .models import Rescue_Time_Query
...
def execute():
    class RescueTimeDataHandling():
        ...
        def _get_key(self):
            #if LOCAL == True:
            try:
                with open("./blog/config.json") as f:
                    print('Found config.json')
                    f.seek(0)
                    _ = json.load(f)
                return _['key']
            except:
                print('Using Environment Variable for Config')
    ...
    for row in df_all.itertuples():
        print(row)
        row = Rescue_Time_Query.objects.create(activity=row.Index, 
